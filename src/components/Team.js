        ...
        <div className="team-container">
          <Slider {...settings}>
            {descriptions.map((member, index) => (
              <div key={index}>
                <div className={`cardd ${index === currentSlide - 1 || (currentSlide === 0 && index === descriptions.length - 1) ? 'prev-cardd' : index === currentSlide + 1 || (currentSlide === descriptions.length - 1 && index === 0) ? 'next-cardd' : ''}`}>
                  <img src={`${process.env.PUBLIC_URL}/images/img${index + 1}.jpg`} alt={`Image ${index + 1}`} />
